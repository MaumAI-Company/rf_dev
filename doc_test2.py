...
async def parse_document_async(session: aiohttp.ClientSession, dataset_id: str, doc_id: str, 
                               doc_name: str = None, timeout: int = PARSING_TIMEOUT) -> Dict:
    """
...
async def parse_documents_sequentially(dataset_id: str, document_infos: List[Dict], 
                                       timeout_per_doc: int = PARSING_TIMEOUT) -> Dict:
    """
    ...
    async with aiohttp.ClientSession(timeout=timeout_obj) as session:
        for idx, doc_info in enumerate(document_infos, 1):
            ...
            # 파싱 요청 및 완료 대기 (순차 처리)
            result = await parse_document_async(session, dataset_id, doc_id, doc_name, timeout=timeout_per_doc)
...
def check_document_status(doc) -> bool:
    """
    문서의 파싱 상태 확인 (SDK의 status 속성 사용)
    
    Args:
        doc: RAGFlow 문서 객체
        
    Returns:
        bool: True면 이미 파싱 완료, False면 파싱 필요
    """
    try:
        # SDK의 문서 객체에서 status 확인
        # status: "1"=파싱 완료, "0"=파싱 대기, "2"=파싱 중
        status = getattr(doc, 'status', None)
        progress = getattr(doc, 'progress', 0)
        
        if status == "1" and progress == 1.0:
            return True  # 이미 파싱 완료
        else:
            return False  # 파싱 필요
            
    except Exception as e:
        print(f"⚠️  Error checking document status: {e}")
        return False  # 에러 시 파싱 필요로 간주

def create_dataset_with_http_api(dataset_name: str) -> str:
    """
    HTTP API를 사용하여 dataset 생성 (이미 존재하는 경우 에러 처리)
    ...
    try:
        ...
        if result.get("code") == 0:
            ...
            print(f"✅ Created new dataset: {dataset_name} (ID: {dataset_id}) with MinerU parser")
            return dataset_id
        elif result.get("code") == 101:
            # Dataset 이미 존재하는 경우
            print(f"⚠️  Dataset '{dataset_name}' already exists")
            # 기존 dataset 목록에서 해당 이름의 dataset ID 찾기
            ...
            if list_result.get("code") == 0:
                ...
                for ds in datasets:
                    if ds.get("name") == dataset_name:
                        ...
                        print(f"✅ Using existing dataset: {dataset_name} (ID: {dataset_id})")
                        return dataset_id
            
            raise Exception(f"Dataset '{dataset_name}' exists but could not retrieve its ID")
        else:
            raise Exception(f"Failed to create dataset: {result.get('message', 'Unknown error')}")
...
def parse_test():
    ...
    # dataset 이름
    dataset_name = "test_mineru_dataset"
    
    # HTTP API로 dataset 생성 또는 기존 dataset 사용
    try:
        dataset_id = create_dataset_with_http_api(dataset_name)
    except Exception as e:
        print(f"❌ Failed to create/get dataset: {e}")
        return
    
    # SDK를 통해 dataset 객체 가져오기
    try:
        datasets = rag_object.list_datasets(id=dataset_id)
        if not datasets:
            print(f"❌ Could not retrieve dataset with ID: {dataset_id}")
            return
        dataset = datasets[0]
    except Exception as e:
        print(f"❌ Error retrieving dataset object: {e}")
        return
    ...
    try:
        ...
        for doc in all_docs:
            # SDK의 status 속성으로 파싱 상태 확인
            if check_document_status(doc):
    ...
    if document_infos:
        ...
        results = asyncio.run(parse_documents_sequentially(
            dataset.id, 
            document_infos, 
            timeout_per_doc=PARSING_TIMEOUT
        ))
